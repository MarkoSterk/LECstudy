...
for exp_name in exp_names:
    ...
    if analysis_type == 'auto':
        ...
        valid_idxs = np.where((~np.isnan(act_times)) & (~np.isnan(deact_times)))[0]
        invalid_idxs = np.where((np.isnan(act_times)) & (np.isnan(deact_times)))[0]
...
plot_items = [
    (act_times_avgs, act_times_stds),
    (response_times_avgs, response_times_stds),
    (amplitudes_avgs, amplitudes_stds),
    (durations_avgs, durations_stds),
    (fraction_avgs, fraction_stds)
]

fig = plt.figure(figsize=(3.0*PANEL_WIDTH, 2.0*PANEL_HEIGHT))
axes = [fig.add_subplot(2,3,i+1) for i in range(5)]
for i, (item, std_errors) in enumerate(plot_items):
    item = np.array(item)
    ...
    x_start, x_stop = 0.94*np.nanmin(dist_from_stim_avgs), 1.02*np.nanmax(dist_from_stim_avgs)
